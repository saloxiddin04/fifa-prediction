...
import {
	FaTimes, FaSearch, FaFutbol,
	FaShieldAlt, FaUserPlus, FaTrash, FaStar,
	FaSpinner, FaExclamationCircle, FaDownload,
	FaChartBar, FaInfoCircle, FaFilter
} from 'react-icons/fa';
import { TbChartRadar } from "react-icons/tb";
import { comparisonAPI, utilityAPI } from '../services/api.js';
...
const PlayerComparison = () => {
	...
	const loadFeatureImportance = async () => {
		try {
			const response = await utilityAPI.getFeatureImportance();
	...
	const loadTopPlayers = async () => {
		try {
			const response = await comparisonAPI.getTopPlayers(20);
	...
	useEffect(() => {
		const searchPlayers = async () => {
			...
			try {
				...
				// Filter by position (frontend filter)
				if (filterPosition !== 'all') {
					results = results.filter(p =>
						(p.MainPosition || p.Position) === filterPosition
					);
	...
	const addTopPlayer = async (player) => {
	...
	const exportPlayerData = async () => {
		...
		try {
			const response = await utilityAPI.exportPlayerData(selectedPlayers.map(p => p.id));

			// Create download link
			const url = window.URL.createObjectURL(new Blob([JSON.stringify(response.data, null, 2)]));
			const link = document.createElement('a');
			link.href = url;
			link.setAttribute('download', `players_comparison_${new Date().getTime()}.json`);
			document.body.appendChild(link);
			link.click();
			link.remove();
		} catch (error) {
			console.error('Error exporting players:', error);
			setError('Failed to export player data');
		}
	};
				...
				<div className="bg-gray-800 text-white p-4 rounded-lg shadow-xl border border-gray-700">
					...
					{payload.map((entry, index) => (
						<div key={index} className="flex items-center justify-between gap-4 py-1">
							...
							<span className="font-bold text-white">{Math.round(entry.value).toFixed(1)}</span>
							{featureImportance?.field && featureImportance.field[entry.payload.fullName] && (
								<span className="text-xs text-gray-400">
									({Math.round(featureImportance.field[entry.payload.fullName] * 100).toFixed(1)}% importance)
								</span>
							)}
						</div>
					))}
	...
	return (
		...
		>
			...
			<AnimatePresence>
				{showFeatureImportance && featureImportance && (
					<motion.div
					...
					>
						<div className="bg-gray-800/80 rounded-xl p-4 border border-gray-700">
							...
							<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
								{Object.entries(featureImportance.field || {}).slice(0, 8).map(([feat, imp]) => (
									<div key={feat} className="text-sm">
										<div className="flex justify-between text-gray-400 mb-1">
											<span>{feat}</span>
											<span>{(imp * 1000).toFixed(1)}%</span>
										</div>
										<div className="w-full bg-gray-700 rounded-full h-1.5">
											<div
												className="bg-blue-500 h-1.5 rounded-full"
												style={{ width: `${imp * 1000}%` }}
											/>
										</div>
									</div>
								))}
						...
						<div className="bg-gray-800/80 rounded-xl p-4 border border-gray-700">
							...
							<div className="grid grid-cols-2 md:grid-cols-5 gap-2">
								{topPlayers.slice(0, 10).map(player => (
									<button
										key={player.ID}
										onClick={() => addTopPlayer(player)}
										className="text-left p-2 bg-gray-700/50 rounded-lg hover:bg-gray-700 transition-colors"
									>
										<div className="text-white font-medium text-sm">{player.Name}</div>
										<div className="text-xs text-gray-400">{player.Overall} OVR • {player.MainPosition}</div>
									</button>
								))}
							</div>
						</div>
				...
				<div className="flex gap-2">
					...
					<div className="relative">
						...
						<select
							value={filterPosition}
							onChange={(e) => setFilterPosition(e.target.value)}
							className="pl-10 pr-8 py-3 bg-gray-800 border border-gray-700 rounded-xl text-white focus:outline-none focus:border-blue-500 appearance-none"
						>
							<option value="all">All Positions</option>
							{positions.map(pos => (
								<option key={pos} value={pos}>{pos}</option>
